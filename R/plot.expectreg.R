        ...
        if (types[[k]] == "pspline") {
            ...
            for (i in 1:np) {
                z = cbind(z, diag(dim(Zspathelp[[k]])[1]) %*% 
                  Zspathelp[[k]] %*% coefficients[[k]] + intercept[i])
            }
            if (class(bnd[[k]]) != "bnd") {
