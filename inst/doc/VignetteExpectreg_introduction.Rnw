...
\begin{table}[ht]
\begin{tabular}{ll}
\texttt{base}& Creates bases for a regression based on covariates\\
\texttt{dkoenker} & Density of a special distribution developed by Roger Koenker \cite{KoenkerSolution}\\
\texttt{ebeta} & Expectiles of the beta distribution\\
\texttt{ekoenker} & Expectiles of a special distribution developed by Roger Koenker\\
\texttt{enorm} & Expectiles of the normal distribution \\
\texttt{eunif} & Expectiles of the uniform distribution \\
\texttt{expectile.boost} & Expectile regression using boosting \\
\texttt{expectile.laws} & Expectiles regression of additive models \\
\texttt{expectile.restricted} & Algorithm proposed by \cite{He1997} for restricted regression quantiles \\
\texttt{expectile.bundle} & Location-scale type model for non-crossing expectiles\\
\texttt{pkoenker}& Distribution function for a special distribution developed by Roger Koenker\\
\texttt{qkoenker}& Quantile function for a special distribution developed by Roger Koenker\\
\texttt{quant.boost} & Quantile regression using boosting\\
\texttt{rkoenker}& Random variable generated from a special distribution developed by Roger Koenker\\
\end{tabular}
\end{table}
...
\end{enumerate}
For expectile regression, the empirical risk is given the asymmetric least squares criterion (\ref{eq:laws}) and the appropriate loss function 
...
The basic function \texttt{expectiles.laws} can be used to estimate 11 expectiles
...
exp.l <- expectile.laws(dutchboys[,3] ~ base(dutchboys[,2],"pspline"),smooth="acv")
@

\begin{center}
\begin{figure}[ht]
\includegraphics[width=10cm]{figures/dutchbsp_laws.pdf}
\caption{Expectile curves estimated using \texttt{expectile.laws}}
\end{figure}
\end{center}
...
<<eval=TRUE, results=hide>>=
exp.b <- expectile.bundle(dutchboys[,3] ~ base(dutchboys[,2],"pspline"),smooth="none")
@

\begin{center}
\begin{figure}[ht]
\includegraphics[width=10cm]{figures/dutchbsp_bundle.pdf}
\caption{Expectile curves estimated using \texttt{expectile.bundle}}
\end{figure}
\end{center}
...
<<eval=TRUE, results=hide>>=
exp.r <- expectile.restricted(dutchboys[,3] ~ base(dutchboys[,2],"pspline"),smooth="schall")
@

\begin{center}
\begin{figure}[ht]
\includegraphics[width=10cm]{figures/dutchbsp_rest.pdf}
\caption{Expectile curves estimated using \texttt{expectile.restricted}}
\end{figure}
\end{center}
...
<<eval=TRUE, results=hide>>= 
exp.boost <- expectile.boost(hgt ~ bbs(age,df=5,degree=2),dutchboys,mstop=rep(500,11)) 
@ 

\begin{center}
\begin{figure}[ht]
\includegraphics[width=10cm]{figures/dutchbsp_boost.pdf}
\caption{Expectile curves estimated using \texttt{expectile.boost}}
\end{figure}
\end{center}
